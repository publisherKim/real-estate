...
### 대체재와 보완재
- 대체재 -> 대신 사용 가능 -> A대신 B를 선택(소비)하는 관계
    - 콜라만 가격 상승 + 사이다(상대적) 가격하락 효과
    - 비싼 콜라 수요량 감소 + 대신 싼 사이다 수요 증가 -> 사이다 가격 상승
- 보완재 -> A와 B를 같이 소비하는 관계
#### 예제
- A 부동산의 가격이 5% 상승할 때, B 부동산의 수요는 10%증가하고 C 부동산의 수요는 15% 감소한다.
    - 이 경우 A와 B는 (대체재) A와 C는 (보완재) 이다.
### 수요고선의 형태(우하향)
- 수요법칙: 가격과 수요량 반비례
- 가격효과: 소득효과 + 대체효과
### 수요량의 변화와 수요의 변화
...
- 수요(공급)의 변화
    ...
    - 곡선이 이동한다.
#### 빅데이터
- 아파트와 단독주택의 관계가 대체재라고 가정할때 아파트의 가격이 상승하면, 대체재인 단독주택의 수요가 증가하고 단독주택의 가격은 상승한다.
- A부동산의 가격이 5% 상승할때, B부동산의 수요는 10%증가하고 C부동산의 수요는 5%감소한다. 이 경우 A와 B는 대체재이고, A와 C는 보완재이다.
- 부동산수요곡선 상 수요량은 주어진 가격수준에서 부동산 구매의사와 구매능력이 있는 수요자가 사전적으로 구매하려고 하는 수량이다.
- 부동산의 수요자의 소득이 변하여 동일한 가격수준에서 부동산의 수요곡선이 이동하였다면 이를 부동산 수요의 변화라 한다.
- 부동산가격은 수요가 감소하더라도 즉각적으로 하락하지 않는 하방경직성의 특성이 있다.

## 1주차 한 줄 마무리
- 부동산학? 순수·자연과학이 아니고 응용·사회·종합과학 이다.
- 속성: 과학성·기술성, 정보·대인 및 대물·임장활동, 장기적 배려, 복합개념
- 복합개념: 물리(유형) + 경제·법률(무형)적 개념
    - 정착물 -> 토지로 부터 독립성 여부 구분
    - 준(의제)부동산 -> 공시방법이 토지·건물과 유사(등기·등록)한 특정의 동산
- 토지와 건물이 하나의 물건으로 취급되어 일체로 이용(거래) -> 복합부동산
- 토지의 분류
    - 후보지: 지역(상호간), 이행지: 지역(내에서)
    - 나지: 건축물(X), 사법상제약(X), 공법상 제약(O)
- 부동성 -> 국지·지역화, 외부효과 발생
- 영속성 -> 감가상각 배제, 장기적 배려
- 부증성
    - 물리적 공급 불가(수직) -> 경제적 공급 가능(우상향) + 부증서 예외(X)
    - 상대적 희소성에 의한 지가고, 집약적 이용, 수요자 경쟁, 토지공개념 등
- 유량: 일정기간 ~ -> 소득, 거래량, 신규주택 공급(생산)량, 가계소비 등
- 저량: 일정시점 ~ -> 가치, 자산, 자본, 인구, 주택재고, 통화량 등
- 부동산수요: 사전적(의도된) + 유효수요(구매력 수반)
- A가격 상승 -> 대체재인 B수요는 (증가), 보완재인 C수요는 (감소)
- 수요법칙: 반비례(-), 공급법칙: 비례(+)
- 수요량의 변화
    - 가격, 곡선이동 없음
- 수요의 변화
